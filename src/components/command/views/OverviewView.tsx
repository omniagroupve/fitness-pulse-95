...
export function OverviewView() {
  ...
  const todayVentas = data.ventas.reduce((sum, item) => sum + item.Monto, 0);
  const totalCostoNomina = data.nomina.reduce((sum, item) => sum + item.Costo, 0);

  // Calculate average response time
  const totalTickets = data.operacionesAtencion.length;
  const onTimeTickets = data.operacionesAtencion.filter(t => t.Tiempo_Respuesta_Minutos <= 5).length;
  const tiempoRespuestaScore = totalTickets > 0 ? Math.round((onTimeTickets / totalTickets) * 100) : 0;

  // Fallbacks to mockdata if no excel is loaded
  const displayVentas = todayVentas > 0 ? todayVentas : estadoGeneral.ventasHoy;
  const displayCostoNomina = totalCostoNomina > 0 ? totalCostoNomina : estadoGeneral.costoNomina;
  const displayTiempoRespuesta = totalTickets > 0 ? tiempoRespuestaScore : atencionCliente.respuestasMenos5Min;
  ...
  const totalVentasMock = ventasPorSede.reduce((sum, s) => sum + s.ventas, 0);
  const ventasTarget = 5500;
  const ventasProgress = (displayVentas / ventasTarget) * 100;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard
            label="Ventas Hoy"
            value={`$${displayVentas.toLocaleString()}`}
          ...
          <MetricCard
            label="Costo Nómina"
            value={`$${displayCostoNomina.toLocaleString()}`}
            status="neutral"
            icon={<Users className="h-4 w-4 text-muted-foreground" />}
          />
          <MetricCard
            label="Tiempo de Respuesta"
            value={`${displayTiempoRespuesta}%`}
            status={displayTiempoRespuesta >= 85 ? 'success' : 'warning'}
